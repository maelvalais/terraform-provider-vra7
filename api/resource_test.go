...
func TestClient_GetCatalogItemRequestTemplate(t *testing.T) {
	client := MockClient()
	httpmock.Activate()
...
func TestClient_RequestCatalogItem(t *testing.T) {
	client := MockClient()
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
...
func TestClient_GetResourceViews(t *testing.T) {
	client := MockClient()
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
...
func TestClient_destroyMachine(t *testing.T) {
	...
	if Error != nil && !Error.IsEmpty() {
		t.Errorf("Expected no error but found %v ", Error.Error())
	}
